...
;;translate symbols from GAMMA to SIGMA, specialized by target: self,
;;A, B, or C neighbor
(define (edge src tar)
...
;;the role emitted by a cell is not the role which affects the
;;behavior of its neighbors
(define circuit 
	(define c0 (rlem3453-buf mem rol initial-sym buf sig))
	(define c1 (rlem3453-buf mem rol (edge c0 1) buf sig))
