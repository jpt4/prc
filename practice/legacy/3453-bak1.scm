...
(define (rlem3453)
	...
	(define terms '(_ _ _ _ _ _))  ;;A B C A' B' C'
	(define nbrs '(_ _ _))         ;;B/A' C/B' A/C'
	...
	(define (state-change cur new)
		(begin
			(case cur
				...
				['terms (set! terms new)]
				['nbrs (set! nbrs new)]
				[else (dispnl 'bad-state-change)])
			(set! state `((id . ,id) (role . ,role) (mem . ,mem) (buf . ,buf)
										(stin . ,stin) (terms . ,terms) (nbrs . ,nbrs)))
			))
	(define (act)
		(case role
